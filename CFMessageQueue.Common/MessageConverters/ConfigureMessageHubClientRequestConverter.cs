...
{
    public class ConfigureMessageHubClientRequestConverter : IExternalMessageConverter<ConfigureMessageHubClientRequest>
    {
        public ConnectionMessage GetConnectionMessage(ConfigureMessageHubClientRequest externalMessage)
        {
            var connectionMessage = new ConnectionMessage()
            {
                ...
                Parameters = new List<ConnectionMessageParameter>()
                {
                     new ConnectionMessageParameter()
                    {
                      ...
                      new ConnectionMessageParameter()
                    {
                        Name = "MessageHubClientId",
                        Value = externalMessage.MessageHubClientId
                    },
                    new ConnectionMessageParameter()
                    {
                        Name = "MessageQueueId",
                    ...
                    {
                        Name = "RoleTypes",
                        Value = String.Join("|", externalMessage.RoleTypes)
                    }
        ...
        public ConfigureMessageHubClientRequest GetExternalMessage(ConnectionMessage connectionMessage)
        {
            var externalMessage = new ConfigureMessageHubClientRequest()
            {
                Id = connectionMessage.Id,
                SecurityKey = connectionMessage.Parameters.First(p => p.Name == "SecurityKey").Value,                
                MessageHubClientId  = connectionMessage.Parameters.First(p => p.Name == "MessageHubClientId").Value,
                MessageQueueId = connectionMessage.Parameters.First(p => p.Name == "MessageQueueId").Value,
                RoleTypes = new()
            };

            foreach(var roleValue in connectionMessage.Parameters.First(p => p.Name == "RoleTypes").Value.Split('|'))
            {
                var roleType = (RoleTypes)Enum.Parse(typeof(RoleTypes), roleValue);
                externalMessage.RoleTypes.Add(roleType);
            }
